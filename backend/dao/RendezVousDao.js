...
class RendezVousDao {
    ...
    async findById(id) {
        const query = 'SELECT * FROM rendez_vous WHERE numero_rdv = ?'; try {
    ...
    async findAllById(id_affilie) {
        ...
        try {
            const [rows] = await connection.query(query, [id_affilie]); return rows.map(row => this._createRendezVousFromRow(row));
        } catch (error) {
    ...
    async update(rendezVous) {
        const query = 'UPDATE rendez_vous SET agence = ?, date_rdv = ?, heure_rdv = ?, type_service = ? ,etat_rdv = ? WHERE numero_rdv = ? ';
        const values = [rendezVous.agence, rendezVous.date_rdv, rendezVous.heure_rdv, rendezVous.type_service, rendezVous.etat_rdv, rendezVous.numero_rdv];
